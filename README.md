...
WebGL 实时渲染：深蓝夜空、被吸引聚合的粒子爱心、蓝色光蝶、
柔和 bloom 与电影式字幕……
...
```js
text: {
  title: '雪 and 潇',                          // Scene 5（and 自动用衬线斜体、更小更淡）
...
| 32.0 – 36.0 | **收尾**：画面稳定约 4 秒，适合录屏最后停留 |

字幕与爱心还有一点轻微互动（复用已有亮度通道，不是新增特效）：
第一句出现时爱心亮度回落约 7% 把注意力让给文字，第二句前后恢复，
最后一句抬升约 8% 并配合那一次光脉冲，读起来像一次呼吸。

---

## 性能说明

本机（Intel Iris Xe 集显）实测 **1920×1080 全时间轴：平均 94 FPS，95% 分位 63 FPS**，
...
node qa/motion.mjs 24           # 把相隔 0.35s 的三帧叠成一张，检查飞行方向与朝向
```
